...
<form action="MemberControllerServlet" method="get">
  ...
  <input type="text" id="search" name="search" value="${param.search}" placeholder="Enter name to search">

  <label for="category">Filter by Category:</label>
  <select id="category" name="category">
    <option value="">Select Category</option>
    <option value="Basic" ${param.category == 'Basic' ? 'selected' : ''}>Basic</option>
    <option value="Premium" ${param.category == 'Premium' ? 'selected' : ''}>Premium</option>
    <option value="VIP" ${param.category == 'VIP' ? 'selected' : ''}>VIP</option>
  </select>
